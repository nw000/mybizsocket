...
import bizsocket.core.MyCode.Server.WPBMockServer;
...
public class WPBPacket extends Packet {
    ...
    public static WPBPacket build(Packet reusable, BufferedSource reader) throws IOException {
        WPBPacket packet = null;
        if (reusable != null && reusable instanceof WPBPacket) {
            packet = (WPBPacket) reusable;
        }
        else {
            packet = new WPBPacket();
        }
